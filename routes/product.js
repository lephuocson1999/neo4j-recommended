...
route.get('/list',ROLE_ADMIN, async (req, res) => {
    ...
    renderToView(req, res, 'dashboard/pages2/list-product', {})
});
...
route.get('/new-product',ROLE_ADMIN, async(req, res) =>{
    ...
    renderToView(req, res, 'dashboard/pages2/add-product',{})
...
route.post('/delete/:id', ROLE_ADMIN,async(req, res)=>{
...
route.get('/update/:id', ROLE_ADMIN, async(req, res)=>{
    ...
    renderToView(req, res, 'dashboard/pages2/update-product',{product : inforProduct.data._fields[0], category :inforProduct.data._fields[1] })
});
...
route.get('/tim-kiem', async (req, res) => {
    ...
    let dataSearch = await PRODUCT_MODEL.findByName(productName);
    console.log({dataSearch});
    
    if(!dataSearch) return res.json({ error : true, message:'cant_search_product' });
    return res.json({ error: false, message : 'success', data: dataSearch.data})
